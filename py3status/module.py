...
class Module(Thread):
    ...
    def process_composite(self, response):
        ...
        for index, item in enumerate(response['composite']):
            ...
            if not self.allow_urgent and 'urgent' in item:
                del item['urgent']
    ...
    def load_methods(self, module, user_modules):
        ...
        if class_inst:
            ...
            # if a non-setting parameter then use the module default/False
            if hasattr(param, 'none_setting'):
                param = getattr(self.module_class, 'allow_urgent', False)
